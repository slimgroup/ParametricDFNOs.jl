...
function grad_test_slim(misfit, x0, dx, g; maxiter=6, h0=5f-2, data=false, stol=1f-1)
    # init
...
function finite_diff_grad_test(J, x, Δx, analytical_grad; ε=1e-6, stol=1e-6)
    n = length(x)
    finite_diff_grad = zeros(n)
    ...
    for i = 1:n
        ...
        finite_diff_grad[i] = (J_plus - J_minus) / (2 * ε)
        diff = abs(finite_diff_grad[i]) - abs(vec(analytical_grad)[i])

        (diff > stol) && println("Norm of the difference between analytical and finite difference gradient @ $i / $n : $diff)")
        # @test isapprox(diff, 0; atol=stol)
    end
end

export grad_test, grad_test_slim, finite_diff_grad_test
