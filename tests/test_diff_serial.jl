...
serial_1 = load(projectdir("data", "3D_FNO", "batch_size=2_dt=0.02_epochs=1_learning_rate=0.0001_modes=4_nt=51_ntrain=1000_nvalid=100_s=1_width=20.jld2"))
serial_2 = load(projectdir("weights", "DFNO_2D", "mt=4_mx=4_my=4_nblocks=1_nc_in=4_nc_lift=20_nc_mid=128_nc_out=1_nt_in=51_nt_out=51_nx=64_ny=64.jld2"))

θ_1 = serial_1["θ_save"]
θ_2 = serial_2["θ_save"]

for (k, v) in θ_1
    println(k, norm(v - θ_2[k]))
    # @assert norm(v - θ_2[k]) <= 1e-8
end

Loss_1 = serial_1["Loss"]
Loss_2 = serial_2["Loss"]

println(norm(Loss_1 - Loss_2))

# @assert norm(Loss_1 - Loss_2) <= 1e-10
