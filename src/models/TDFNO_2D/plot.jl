...
function plotEvaluation(modelConfig::ModelConfig, trainConfig::TrainConfig, x_plot, y_plot, y_predict; additional=Dict{String,Any}())
    ...
    for i = 1:5
        subplot(4,5,i)
        imshow(x_plot[1,2*i+1,:,:]')
        title("x")

        subplot(4,5,i+5)
        imshow(y_plot[1,2*i+1,:,:]', vmin=0, vmax=1)
        title("true y")

        subplot(4,5,i+10)
        imshow(y_predict[1,2*i+1,:,:]', vmin=0, vmax=1)
        title("predict y")

        subplot(4,5,i+15)
        imshow(5f0 .* abs.(y_plot[1,2*i+1,:,:]'-y_predict[1,2*i+1,:,:]'), vmin=0, vmax=1)
