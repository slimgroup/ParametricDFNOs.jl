...
mutable struct Model
    ...
    function Model(config::ModelConfig)
        ...
        function spectral_convolution(layer::Int)
            ...
            weight_mix = ParTensor(Complex{T}, weight_order, weight_shape, input_order, input_shape, target_order, input_shape, "ParMatrixN_SCONV:($(layer))")
...
function print_storage_complexity(config::ModelConfig; batch=1)
    ...
    output = @sprintf("DFNO_2D_OLD (batch=%d) | ~ %.2f MB for weights | ~ %.2f GB for forward pass | ~ %.2f GB for backward pass | Total ~ %.2f GB |", batch, w_mb, c_gb, g_gb, t_gb)
