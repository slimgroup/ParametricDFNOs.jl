...
function train!(config::TrainConfig, model::Model, θ::Dict; comm=MPI.COMM_WORLD)
    ...
    ntrain = size(config.x_train, 6)
    nvalid = size(config.x_valid, 6)
    ...
    x_global_shape = (model.config.nc_in, model.config.nx, model.config.ny, model.config.nz, model.config.nt)
    y_global_shape = (model.config.nc_out, model.config.nx, model.config.ny, model.config.nz, model.config.nt)
